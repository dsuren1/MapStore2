...
import { INTERACTIVE_LEGEND } from './LegendUtils';
let FilterUtils;
...
export const updateLayerLegendFilter = (layerFilterObj, legendFilter) => {
    ...
    const isLegendFilterExist = filterObj?.filters?.find(f => f.id === INTERACTIVE_LEGEND);
    if (!legendFilter) {
        // clear legend filter with id = 'interactiveLegend'
        if (isLegendFilterExist) {
            filterObj = {
                ...filterObj, filters: filterObj?.filters?.filter(f => f.id !== INTERACTIVE_LEGEND)
            };
        }
    ...
    let newFilter = {
        ...(filterObj || {}), filters: [
            ...(filterObj?.filters?.filter(f => f.id !== INTERACTIVE_LEGEND) || []), ...[
                {
