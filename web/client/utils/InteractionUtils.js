 ...
 * It is always present for discovery, but is enabled only when a sibling
 * `applyFilter` interaction targets a layer in the same map.
...
/**
 * Returns true when the interaction target points to the main map's zoom-to field (map.zoom-to).
 * @param {string} nodePath the node path to check
 * @returns {boolean}
 */
 ...
 * (widgets[widgetId].maps[mapId].zoom-to).
 * @param {string} nodePath the node path to check
 * @returns {boolean}
 */
 ...
 * so the caller can compute a union extent.
 ...
 * @returns {array} Array of matching applyFilter interactions (may be empty)
 */
