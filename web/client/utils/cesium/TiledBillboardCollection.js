 ...
 *   using a shared BillboardCollection, or 'feature' for all geometry types using GeoJSONStyledFeatures per tile
    ...
    this._update = () => {
        ...
        timeout = setTimeout(() => {
            ...
            if (!this._removed) {
                ...
                let target = this._map.scene.globe.pick(
                    new Cesium.Ray(this._map.camera.position, this._map.camera.direction),
                    this._map.scene
                );
                let tiles = [];
                if (target) {
                    ...
                    if (imageryLevel >= this._minimumLevel) {
                        ...
                        for (let y = topLeftTile.y; y < bottomRightTile.y + 1; y++) {
                            for (let x = topLeftTile.x; x < bottomRightTile.x + 1; x++) {
                                ...
                                tiles.push({
                                    callId: this._callId,
                                    distance: Cesium.Cartesian2.distance(
                                        new Cesium.Cartesian2(centerTile.x, centerTile.y),
                ...
                tiles.forEach(tile => {
                    ...
                    if (!this._tileCache[tile.id]) {
                        this._tileCache[tile.id] = this._createTile(tile, options);
                        this._loadTile(tile)
                            .then(({ features }) =>
...
TiledBillboardCollection.prototype._createTile = function(tile, options) {
    if (this._tileType === 'billboard') {
