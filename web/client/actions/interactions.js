...
/**
 * Manually trigger a zoomTo interaction effect for a filter widget's filter, regardless of the
 * `autoZoom` configuration. Used by the manual zoom button rendered on the runtime filter widget.
 * @param {string} widgetId - The filter widget ID
 * @param {string} [target='floating'] - The target container (defaults to 'floating')
 * @param {string} filterId - The filter ID whose plugged zoomTo interaction(s) should zoom
 */
