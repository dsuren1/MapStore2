...
const previewContents = {
    title: () => null,
    paragraph: ({ id, contents, isCollapsed, scrollTo, onSort, sectionId, onUpdate, onDuplicateContent }) => (
                            ...
                            buttons={[
                                {
                                    ...
                                    onClick: (evt) => {
                                        evt.stopPropagation();
                                        if (onDuplicateContent) onDuplicateContent(containerPath, content.id);
                                    }
                                },
                                {
                                    glyph: 'zoom-to',
    ...
    column: ({ sectionId, id, contents, isCollapsed, scrollTo, onSort, onUpdate, sectionType, onDuplicateContent }) => (
        <div style={{ position: 'relative' }}>
            <DraggableSideGrid
                containerId={id}
                isDraggable
                ...
                items={contents.map((content) => {
                    ...
                    return {
                        ...
                        body: !isCollapsed && PreviewContents
                            && <PreviewContents id={id} onSort={onSort} onDuplicateContent={onDuplicateContent}/>
                    };
                })} />
        </div>
    ),
    immersive: ({ id, contents, isCollapsed, scrollTo, onUpdate, onSort, currentPage, onDuplicateContent }) => (
        <div style={{ position: 'relative' }}>
            <DraggableSideGrid
                containerId={id}
                isDraggable
    ...
    carousel: ({ id, contents, isCollapsed, scrollTo, onUpdate, onSort, currentPage, onDuplicateContent }) => (
        <div style={{ position: 'relative' }}>
            <DraggableSideGrid
                containerId={id}
                isDraggable
 ...
 */
export default ({ sections = [], scrollTo, onSelect = () => {}, isCollapsed, currentPage, selected, onSort, onUpdate, onDuplicate, onDuplicateContent }) => (<DraggableSideGrid
    isDraggable
    onSort={(sortIdTo, sortIdFrom, itemDataTo, itemDataFrom) => {
    ...
    size="sm"
    items={
        sections.map(sectionToItem({ currentPage, onSelect, isCollapsed, scrollTo, selected, onUpdate, onSort, onDuplicate, onDuplicateContent }))
    } />);
