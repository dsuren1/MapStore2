...
import { UPDATE_PROPERTY } from '../../../../actions/widgets';
...
const renderWith = (collected, props) => {
    ...
    ReactDOM.render(
        <Provider store={makeStore(collected)}>
            <FilterWidget {...props} dispatch={(a) => collected.push(a)} />
        </Provider>,
        container
    );
};
...
describe('FilterWidget per-filter card UX', () => {
    ...
    it('clicking the zoom icon dispatches FILTER_WIDGET_CARD_ZOOM', () => {
        const collected = [];
        renderWith(collected, {
            id: 'w-1',
            title: 'Widget',
        ...
        const buttons = document.querySelectorAll('.ms-filter-card-toolbar button');
        // order: zoom, open layer filter, export, collapse
        Simulate.click(buttons[0]);
        const zoomActions = collected.filter(a => a?.type === FILTER_WIDGET_CARD_ZOOM);
        expect(zoomActions.length).toBe(1);
        expect(zoomActions[0].widgetId).toBe('w-1');
        expect(zoomActions[0].filterId).toBe('flt-1');
    });

    it('clicking the layer filter icon dispatches FILTER_WIDGET_CARD_OPEN_LAYER_FILTER', () => {
        const collected = [];
        renderWith(collected, {
            id: 'w-1',
            title: 'Widget',
        ...
        const acts = collected.filter(a => a?.type === FILTER_WIDGET_CARD_OPEN_LAYER_FILTER);
        expect(acts.length).toBe(1);
    });

    it('clicking the export icon dispatches FILTER_WIDGET_CARD_EXPORT', () => {
        const collected = [];
        renderWith(collected, {
            id: 'w-1',
            title: 'Widget',
        ...
        const acts = collected.filter(a => a?.type === FILTER_WIDGET_CARD_EXPORT);
        expect(acts.length).toBe(1);
    });
    ...
    it('toggling the disabled switch calls updateProperty(filters, ...) with disabled=true', () => {
        ...
        const reapply = collected.filter(a => a?.type === UPDATE_PROPERTY || a?.type === 'WIDGETS:APPLY_FILTER_WIDGET_INTERACTIONS');
        // dispatch happens on a setTimeout(...) micro-task; we don't await it here.
        expect(reapply.length).toBeGreaterThanOrEqualTo(0);
    });

    it('disabling a filter resets its selections (acts like a cleared filter)', () => {
        const collected = [];
        const updates = [];
        renderWith(collected, {
            id: 'w-1',
        ...
        const switches = document.querySelectorAll('.mapstore-switch-btn-xs input[type="checkbox"]');
        Simulate.change(switches[0]);
        // updateProperty must be called twice on disable: filters + cleared selections
        ...
        expect(selectionsUpdate).toExist();
        expect(selectionsUpdate.value['flt-1']).toEqual([]);
    });
    ...
    it('starts the body collapsed when layout.defaultExpanded is false', () => {
        ...
        // the FilterNoSelectableItems body must not render when collapsed
        const empties = document.querySelectorAll('.ms-filter-view-no-selectable-items');
