...
const hasLayer = (filterData) => !!filterData?.data?.layer;
const isFeaturesSource = (filterData) => filterData?.data?.dataSource === 'features';

/**
 * Per-filter card toolbar shown in the FilterView header. Supports:
 * - collapse / expand the filter body
 * - zoom to current filter results
 * - open the per-layer filter panel for the selected layer
 * - export filtered data (opens existing LayerDownload dialog)
 * - enable / disable the filter (skipped from interaction CQL composition)
 */
const FilterPerItemToolbar = ({
    filterData,
    collapsed = false,
    onToggleCollapse,
    onZoom,
    ...
    const layerAvailable = hasLayer(filterData);
    const featuresLayer = isFeaturesSource(filterData) && layerAvailable;
        ...
        >
            ...
            {onZoom && (
                <ToolButton
                    ...
                    disabled={!enabled || !featuresLayer}
                    onClick={onZoom}
                />
            )}
            {onOpenLayerFilter && (
                <ToolButton
                    ...
                    tooltipId={`flt-o-${filterData?.id}`}
                    disabled={!featuresLayer}
...
FilterPerItemToolbar.propTypes = {
    filterData: PropTypes.object,
    collapsed: PropTypes.bool,
    onToggleCollapse: PropTypes.func,
    onZoom: PropTypes.func,
    onOpenLayerFilter: PropTypes.func,
    onExport: PropTypes.func,
    onToggleDisabled: PropTypes.func
};

export default FilterPerItemToolbar;
