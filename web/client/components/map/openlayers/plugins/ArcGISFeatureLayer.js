...
const createLoader = (source, options) => (extent, resolution, projection) => {
    ...
    fetchAllPages(buildQueryUrl(options), params, options.security?.sourceId, options.maxRecordCount)
        .then(collection => {
            source.addFeatures(
                source.getFormat().readFeatures(collection, {
...
Layers.registerType('arcgis-feature', {
    create: (options, map) => {
        ...
        source.setLoader(
            createLoader(source, {
                ...options,
                onLoadEnd: () => updateStyle(layer, options, map)
            })
        );
